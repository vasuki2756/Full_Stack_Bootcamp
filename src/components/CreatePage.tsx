...
type CreatePageProps = {
  ...
  setNewCapsule: (capsule: Omit<Capsule, 'id' | 'createdAt' | 'isLocked'>) => void;
  createCapsule: () => void;
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 border border-white/20">
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-5 gap-3">
              {['hopeful', 'grateful', 'excited', 'nostalgic', 'dreamy'].map((mood) => (
                <button
                  key={mood}
                  onClick={() => setNewCapsule({ ...newCapsule, mood: mood as Capsule['mood'] })}
                ...
                >
                  <div className="text-2xl mb-1">{getMoodEmoji(mood as Capsule['mood'])}</div>
          ...
          <div className="mb-8">
            ...
            <div className="flex gap-3">
              {['blue', 'pink', 'green', 'orange', 'purple'].map((color) => (
                <button
                  key={color}
                  onClick={() => setNewCapsule({ ...newCapsule, color: color as Capsule['color'] })}
                  className={`w-12 h-12 rounded-full bg-gradient-to-r ${getColorClasses(color as Capsule['color'])} border-4 transition-all duration-300 ${
